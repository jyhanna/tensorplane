...
class Template:
    ...
    def test_index_instance_sorting(self, d, f):
        ...
        np_fn = lambda a: a[array(ids),:]

        res = np_convert(d[ids,:].tensors)
        tgt = np_convert(d.tensors, fn=np_fn)
        assert_all_eq(zip(res,tgt),
                     'Incorrect index full dataset sorting using argsort')

        if d.size[1] < 3: return

        res = np_convert(d[ids,[d.a,d.b]].tensors) + np_convert([d.c])
        tgt = np_convert([d.a,d.b], fn=np_fn) + np_convert([d.c])
        assert_all_eq(zip(res,tgt),
                     'Incorrect index feature subset sorting using argsort')

    def test_index_instance_shuffling(self):
        pass

    def test_index_instance_slices(self):
        pass
